...
namespace OLM.ApiGateWays.SPA.Web.Gtw.SPA.Tests.Aggregator.BundleMachine.OneMachine.Daily
{
    public class FetchDailyBundlesMethodTests
    {
        [Fact]
        public async Task Fetch_ShouldBeOk()
        {
            ...
            mockedFetchMachine.Setup(m => m.FetchDailyBundle(machineID)).ReturnsAsync(new SummarizedBundlesForMachineDataViewModel(0, 0, 0, 0, machineID, bundleIDs));
            ...
            //Act
            var aggregator = new DailyBundlesWithBundlePriceForMachineAggregator(mockedFetchMachine.Object, mockedTCOCalc.Object, mockedBundlePrice.Object);
            var result = (await aggregator.FetchDailyBundles(machineID)).Value;

            //Assert
            Assert.NotNull(result);

            mockedFetchMachine.Verify(m => m.FetchDailyBundle(machineID), Times.Once());
        ...
        public async Task Fetch_BundleReturnedNull()
        {
            ...
            var mockedFetchMachine = new Mock<IFetchOneMachinesBundleService>();
            mockedFetchMachine.Setup(m => m.FetchDailyBundle(machineID)).ReturnsAsync(new APIError());
            var mockedTCOCalc = new Mock<IFetchRawTCOCalculatorService>();
            mockedTCOCalc.Setup(m => m.CalculateAVGTCO(expectedPrices)).ReturnsAsync(new BundleTCOAPIResponseViewModel(0, 0, 0));
            var mockedBundlePrice = new Mock<IFetchBundlePriceService>();
            mockedBundlePrice.Setup(m => m.Fetch(bundleIDs)).ReturnsAsync(expectedPrices);
