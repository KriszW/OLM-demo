...
{
    public class RoutingTimeUpdaterBackgroundService : BackgroundService
    {
        private const int WaitingTimeInMSBetweenUpdates = 30000;

        private ILogger<RoutingTimeUpdaterBackgroundService> _logger;
        private IHostApplicationLifetime _applicationLifetime;
        private IUpdaterService _uploaderService;

        public RoutingTimeUpdaterBackgroundService(ILogger<RoutingTimeUpdaterBackgroundService> logger,
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (stoppingToken.IsCancellationRequested == false)
            {
                ...
                {
                    _logger.LogError(ex, $"A rakatok frissítése közben nem várt hiba lépett fel, ez a frissítési ciklus kimarad: {ex.Message}");
                }
