...
namespace OLM.Services.RoutingTime.Tests.API.Controllers.ProductionTime
{
    public class GetByIDActionTests
    {
        ...
        public async void ShouldReturnSuccess()
        {
            ...
            Assert.IsType<ActionResult<APIResponse<ProductionTimeModel>>>(actionResult);
            Assert.IsType<OkObjectResult>(actionResult.Result);
            var requestObject = Assert.IsAssignableFrom<ObjectResult>(actionResult.Result);
            var actual = Assert.IsAssignableFrom<APIResponse<ProductionTimeModel>>(requestObject.Value);
        ...
        public async void ShouldReturnNotFound()
        {
            ...
            var id = 100;
            var expectedMSG = $"A {id} azonosítóval nincs feltöltve nyitva tartási idő az adatbázisba";
