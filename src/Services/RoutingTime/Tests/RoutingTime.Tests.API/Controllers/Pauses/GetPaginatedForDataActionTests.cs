...
namespace OLM.Services.RoutingTime.Tests.API.Controllers.Pauses
{
    public class GetPaginatedForDataActionTests
    {
        ...
        public async void ShouldReturnSuccess(int pageIndex, int pageSize, int expecetedTotalCount)
        {
            ...
            Assert.IsType<ActionResult<APIResponse<Paginated<PauseModel>>>>(actionResult);
            Assert.IsType<OkObjectResult>(actionResult.Result);
            var requestObject = Assert.IsAssignableFrom<ObjectResult>(actionResult.Result);
            var actual = Assert.IsAssignableFrom<APIResponse<Paginated<PauseModel>>>(requestObject.Value);
        ...
        {
            new object[] { 30,30, "A megadott adatokkal nincs feltöltve szünet az adatbázisba" },
        };
    }
}
