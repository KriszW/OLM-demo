...
namespace OLM.Services.RoutingTime.Tests.API.Controllers.Pauses
{
    public class GetByIDActionTests
    {
        ...
        public async void ShouldReturnSuccess()
        {
            ...
            Assert.IsType<ActionResult<APIResponse<PauseModel>>>(actionResult);
            Assert.IsType<OkObjectResult>(actionResult.Result);
            var requestObject = Assert.IsAssignableFrom<ObjectResult>(actionResult.Result);
            var actual = Assert.IsAssignableFrom<APIResponse<PauseModel>>(requestObject.Value);
        ...
        public async void ShouldReturnNotFound()
        {
            ...
            var id = 100;
            var expectedMSG = $"A {id} azonosítóval nincs feltöltve szünet az adatbázisba";
