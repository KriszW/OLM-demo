...
namespace OLM.Services.Tram.API.Tests.Controllers.TramData
{
    public class GetByIDActionTests
    {
        ...
        public async void ShouldReturnSuccess()
        {
            ...
            Assert.IsType<ActionResult<APIResponse<TramDataModel>>>(actionResult);
            Assert.IsType<OkObjectResult>(actionResult.Result);
            var requestObject = Assert.IsAssignableFrom<ObjectResult>(actionResult.Result);
            var actual = Assert.IsAssignableFrom<APIResponse<TramDataModel>>(requestObject.Value);
        ...
        public async void ShouldReturnNotFound()
        {
            ...
            var id = 100;
            var expectedMSG = $"A {id} azonosítóval nincs feltöltve csille adat az adatbázisba";
